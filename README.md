...
- `agents/` — Python multi-agent backend. 4 specialists + Fold ensemble (confidence-weighted consensus) + orchestrator + backtest harness. DeepSeek (deepseek-v4-pro) backend with native chain-of-thought streaming. 45/45 Python tests passing (`pytest` from repo root).
...
- **Live chain-of-thought** visible per agent (deepseek-v4-pro's `reasoning_content` channel maps directly to a visible thinking stream).
